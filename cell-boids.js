...
const main = () => {
    ...
    let particleUpdateProgram;
    {
        ...
        particleUpdateProgram = {
            ...
            uniformUpload: {
                ...
                cellDim: (w, h) => gl.uniform2f(gl.getUniformLocation(program, 'cellDim'), w, h),
                cellTex: texID => gl.uniform1i(gl.getUniformLocation(program, 'cellTex'), texID),
    ...
    let cellUpdateProgram;
    {
        const program = createProgram(gl, 'update-vs', 'cell-update-fs');
        cellUpdateProgram = {
            program: program,
            attribs: {
                pos: {
        ...
        gl.bindVertexArray(cellUpdateProgram.vao);
        gl.bindBuffer(gl.ARRAY_BUFFER, cellUpdateProgram.buffers.pos);
        {
            const pos = particleUpdateProgram.attribs.pos;
            gl.enableVertexAttribArray(pos.loc);
            gl.vertexAttribPointer(
    ...
    const cellSizesSlider = document.getElementById("numCellsSlider");
    let [cellsX, cellsY] = cellSizes[cellSizesSlider.value];
    let numParticles, ins, outs, cell;

    const sortCellData = (data) => {
        data.sort((a, b) => a[1] - b[1]);
        // update cell's first pos
        for (let i = 0; i < numParticles; i++) {
            // if the current particle's cell id start not been set, use i
            if (data[data[i][1]][2] === -1) data[data[i][1]][2] = i;
        }
    }

    const setupTextures = () => {
        if (texWidth * texHeight < cellsX * cellsY) alert("error! more cells than particles, increase particles and refresh.");
        ...
        const cellData = [];
        for (let i = 0; i < numParticles; i++) {
            const cellX = Math.floor(cellsX * posVel[4 * i] / gl.canvas.width);
            const cellY = Math.floor(cellsY * posVel[4 * i + 1] / gl.canvas.height);
            cellData.push([i, cellY * cellsX + cellX, -1, 0]);
        }
        sortCellData(cellData)

        const cellTex = createTexture(gl, 4, gl.INT, new Int32Array(cellData.flat()), texWidth, texHeight);
        ...
        cell = {
            fb: createFrameBuffer(gl, cellTex),
            tex: cellTex
        }
    }
    ...
    cellSizesSlider.oninput = () => {
        [cellsX, cellsY] = cellSizes[cellSizesSlider.value]
        setupTextures();
    }
    ...
    const render = elapsedMillis => {
        ...
        fps_txt.textContent = `${numParticles} (${cellsX}x${cellsY} cells) @ ${Math.round(fps).toString()}fps`;
        ...
        gl.bindTexture(gl.TEXTURE_2D, cell.tex);
        ...
        particleUpdateProgram.uniformUpload.cellDim(cellsX, cellsY);
        particleUpdateProgram.uniformUpload.cellTex(1);

        gl.drawArrays(gl.TRIANGLES, 0, 6);

        // cell update
        gl.bindFramebuffer(gl.FRAMEBUFFER, cell.fb);
        gl.viewport(0, 0, texWidth, texHeight);
        ...
        gl.useProgram(cellUpdateProgram.program);
        cellUpdateProgram.uniformUpload.posVelTex(0);
        cellUpdateProgram.uniformUpload.canvasDim(gl.canvas.width, gl.canvas.height);
        cellUpdateProgram.uniformUpload.cellDim(cellsX, cellsY);

        gl.drawArrays(gl.TRIANGLES, 0, 6);

        const cellData = readFB(gl, cell.fb, gl.INT, 4, texWidth, texHeight);
        {
            const data = [];
            for (let i = 0; i < numParticles; i++) {
                data.push([cellData[4*i], cellData[4*i+1], -1, 0]);
            }
            sortCellData(data)
            gl.bindTexture(gl.TEXTURE_2D, cell.tex);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32I, texWidth, texHeight, 0, gl.RGBA_INTEGER, gl.INT, new Int32Array(data.flat()));
        }
    ...
    window.addEventListener('keydown', e => {
        if (e.code === 'Space') {logging = true; requestAnimationFrame(render);}
        if (e.code === 'KeyS') {logging = false; requestAnimationFrame(render);}
    })
    requestAnimationFrame(render);
};
